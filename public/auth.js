(function(){
  ...
  async function getSession(){
    try{
      ...
      if(e.status===401){currentSession=null;return null;}
      throw e;
    }
  }
  ...
  window.StockHeavenAuth={session:()=>currentSession,role,isAdmin,isGuest,logout,loginAdmin,loginGuest,guestCredentials,saveGuestCredentials,getLoginLog,clearLoginLog,restrictions:getRestrictions,saveRestrictions,isPageAllowedForGuest:async file=>{const r=await getRestrictions();return !r[file]}};

  function injectTopbar(){
    const header=document.querySelector('.top-header');
    if(!header||document.querySelector('.auth-topbar')||!currentSession)return;
    ...
    if(currentSession.role==='admin'){
      const a=document.createElement('a');a.className='auth-admin-link';a.href='admin.html';a.innerHTML='<i class="fa-solid fa-shield-halved"></i> Admin';box.append(a);
    }else{
      const timer=document.createElement('span');timer.className='auth-user-badge';timer.id='guestSessionTimer';box.append(timer);
    }
  ...
  function startGuestTimer(){
    if(!isGuest()||!currentSession.expiresAt)return;
    const expiry=new Date(currentSession.expiresAt).getTime();
    if(sessionTimer)clearInterval(sessionTimer);
    const tick=()=>{
      const el=document.getElementById('guestSessionTimer');
      const left=Math.max(0,expiry-Date.now());
      if(el)el.textContent='Logout in '+Math.ceil(left/1000)+'s';
      if(left<=0){clearInterval(sessionTimer);sessionTimer=null;logout();}
    };
  ...
  function showAuthError(message){
    ...
    document.body.innerHTML='<div class="auth-page-lock"><div class="auth-page-lock-card"><i class="fa-solid fa-triangle-exclamation"></i><h2>Authentication Error</h2><p>'+String(message).replace(/[&<>]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;'}[c]))+'</p><button onclick="location.reload()">Retry</button></div></div>';
  }
  ...
  function lockPage(message='Page Restricted'){
    ...
    window.addEventListener('DOMContentLoaded',()=>{
      document.documentElement.style.visibility='visible';
      document.body.innerHTML='<div class="auth-page-lock"><div class="auth-page-lock-card"><i class="fa-solid fa-lock"></i><h2>'+message+'</h2><p>Admin ne is page ko Guest ke liye restrict kiya hai.</p><a href="index.html">Back to Dashboard</a></div></div>';
    },{once:true});
  }
  ...
  function disableGuestEditing(){
    ...
    const apply=()=>{
      selectors.forEach(sel=>document.querySelectorAll(sel).forEach(el=>{
        el.disabled=true;el.classList.add('guest-edit-disabled');el.setAttribute('aria-disabled','true');
        el.onclick=(e)=>{e.preventDefault();e.stopPropagation();alert('Guest mode mein editing allowed nahi hai.');return false;};
      }));
      document.querySelectorAll('[draggable="true"]').forEach(el=>{el.setAttribute('draggable','false');el.classList.add('guest-edit-disabled')});
    };
    apply();
  ...
  async function protect(){
    ...
    try{
      ...
      if(s.role==='guest'&&pages[currentFile]){
        ...
        if(restrictions[currentFile]){lockPage();return;}
      }
      ...
      showAuthError('Server authentication service se response nahi mil raha. Page ko refresh karke dobara try karein.');
    }
  }
  ...
  if(currentFile==='login.html'){
    window.addEventListener('DOMContentLoaded',async()=>{
      ...
      try{
        ...
        // If the auth service is temporarily unavailable, keep the login screen visible.
        err.textContent='Server se connection nahi ho pa raha. Dobara try karein.';
      }
