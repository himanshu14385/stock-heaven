...
async function getTicker(market){
 ...
 // /api/crypto/ticker returns { results: <object> } for a single market.
 // The old code checked d.market directly, so every valid single-coin
 // response was incorrectly treated as an invalid market price.
 const ticker=d.results && !Array.isArray(d.results) ? d.results : d;
 if(!ticker || !ticker.market)throw Error('CoinDCX did not return a valid market price. Please try again.');
 return ticker;
}
