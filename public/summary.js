...
async function showSummaryQuote(symbol){
    ...
    try{
      ...
      const price=Number(d.price), d20=sma(h,20), d50=sma(h,50), d200=sma(h,200), rsi=rsi14(h), avgVol=sma(vols,20), recent=h.slice(-20);
      ...
      let score=0; if(above20)score+=20; if(above50)score+=20; if(d200!==null&&above200)score+=20; if(rsi!==null){if(rsi>=50&&rsi<=70)score+=20;else if(rsi>=40&&rsi<50)score+=12;else if(rsi>70)score+=8;} if(avgVol!==null&&Number(d.volume)>avgVol)score+=20; score=Math.round(score);
      ...
      document.getElementById("whyBuy").innerHTML=(above20?["Price is above 20 DMA."]:[]).concat(above50?["Price is above 50 DMA."]:[]).concat(rsi!==null&&rsi>=50&&rsi<=70?["RSI is in a relatively healthy zone."]:[]).concat(Number(d.volume)>avgVol?["Volume is above its 20-day average."]:[]).map(x=>`<li>${x}</li>`).join("")||"<li>Technical setup does not currently provide a strong positive signal.</li>";
      document.getElementById("whyNot").innerHTML=(d200!==null&&!above200?["Price is below 200 DMA."]:[]).concat(rsi!==null&&rsi>70?["RSI is in overbought territory."]:[]).concat(rsi!==null&&rsi<40?["RSI is weak."]:[]).concat(!above20?["Price is below 20 DMA."]:[]).map(x=>`<li>${x}</li>`).join("")||"<li>No major technical warning detected from the available data.</li>";
      document.getElementById("topRisks").innerHTML=(d200!==null&&!above200?["Long-term trend is below 200 DMA."]:[]).concat(rsi!==null&&rsi>70?["Short-term overbought condition."]:[]).concat(rsi!==null&&rsi<40?["Weak momentum."]:[]).concat(nearHigh!==null&&nearHigh<5?["Price is close to the 52-week high."]:[]).map(x=>`<li>${x}</li>`).join("")||"<li>Continue monitoring price, volume and market conditions.</li>";
