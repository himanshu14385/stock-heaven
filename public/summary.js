...
function renderSummarySuggestions(matches){
  const box=document.getElementById("stockSuggestions");
  if(!box) return;
  const clean=matches.slice(0,10);
  box.innerHTML=clean.map(s=>{
    const symbol=escapeHtml(s.symbol);
    const name=escapeHtml(s.name);
    return `<div class="stock-suggestion" onclick="showSummaryQuote('${String(s.symbol).replace(/'/g,"\\'")}')">
      <div class="suggestion-icon"><i class="fa-solid fa-chart-line"></i></div>
      <div class="suggestion-info">
        <span class="suggestion-name">${name}</span>
        <span class="suggestion-symbol">NSE · <strong>${symbol}</strong></span>
      </div>
    </div>`;
  }).join("");
  box.style.display=clean.length?"block":"none";
}
...
function showSummarySuggestions(){
  ...
  const requestId=++summarySearchRequest;
  clearTimeout(summarySearchTimer);
  summarySearchTimer=setTimeout(async()=>{
    try{
      const remote=await fetchStockSearch(q);
      if(requestId!==summarySearchRequest) return;
      if(remote.length){
        renderSummarySuggestions(remote);
      }else{
        const uq=q.toUpperCase();
        const fallback=summaryStockList
          .filter(s=>s[0].toUpperCase().includes(uq)||s[1].toUpperCase().includes(uq))
          .map(s=>({symbol:s[0],name:s[1]}));
        renderSummarySuggestions(fallback);
      }
    }catch(e){
      if(requestId!==summarySearchRequest) return;
      const uq=q.toUpperCase();
...
function handleSummarySearch(e){
...
async function getPeerData(symbol){
  try{
    ...
    return {price:Number(d.price), rsi:rsi14(closes)};
  }catch(e){ return null; }
}
